...
export function nearestSchedule(time, occurredAt) {
  ...
  return [-1, 0, 1]
    ...
    .sort(
      (a, b) =>
        Math.abs(a - new Date(occurredAt)) - Math.abs(b - new Date(occurredAt)),
    )[0];
}

export function isOnTime(time, occurredAt) {
  return (
    Math.abs(new Date(occurredAt) - nearestSchedule(time, occurredAt)) <=
    30 * 60 * 1000
  );
}
...
export function companionDiffers(current, reported) {
  return Boolean(
    reported &&
    (current.level !== reported.level ||
      current.experience !== reported.experience),
  );
}
...
export function medicationHeadline(schedules, records, now = new Date()) {
  ...
  const status = (id) =>
    records.find((record) => record.date === today && record.scheduleId === id)
      ?.status ?? "pending";
  const completed = schedules
    .filter((schedule) => status(schedule.id) === "done")
    .sort((a, b) => a.time.localeCompare(b.time))
    .at(-1);
  const missed = schedules
    .filter((schedule) => status(schedule.id) === "missed")
    .sort((a, b) => a.time.localeCompare(b.time))
    .at(-1);
  const next = schedules.find((schedule) => status(schedule.id) === "pending");
  let prefix = "";
  if (missed) {
    prefix = `${missed.name.replace(/\s*약$/, "")} 미복용, `;
  } else if (completed) {
    prefix = `${completed.name.replace(/\s*약$/, "")} 복용 완료, `;
  }

  if (!next) {
    if (missed) return `${prefix}오늘 남은 복약이 없어요.`;
    if (completed) return `${prefix}오늘 복약을 모두 마쳤어요.`;
    return "오늘 예정된 복약이 없어요.";
  }
  const minutes = Math.max(
    0,
    Math.ceil((scheduledAt(next.time, now) - now) / 60000),
  );
...
export function missedSchedules(schedules, records, now = new Date()) {
  const today = localDateKey(now);
